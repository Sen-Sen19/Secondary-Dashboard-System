...
#planTable td:not(:nth-child(-n+5)),
#planTable th:not(:nth-child(-n+5)) {
  z-index: 1;
}
...
<thead>
  <tr>
    ...
    <th style="min-width: 130px; z-index: 12 !important;">Skill Level</th>
...
<th style="min-width: 140px; z-index: 11 !important">7</th>
<th style="min-width: 140px; z-index: 11 !important">8</th>
<th style="min-width: 140px; z-index: 11 !important">9</th>
<th style="min-width: 130px; z-index: 11 !important">10</th>
                                    <th>11</th>
                                    <th>12</th>
                                    <th>13</th>
                                    <th>14</th>
...
// Sort by custom process order first, then by machine_no
data.sort((a, b) => {
    const normalize = str => str.trim().toLowerCase();

    // Sorting by process order
    const aIndex = customProcessOrder.findIndex(p => normalize(p) === normalize(a.process));
    const bIndex = customProcessOrder.findIndex(p => normalize(p) === normalize(b.process));

    const safeA = aIndex === -1 ? 9999 : aIndex;
    const safeB = bIndex === -1 ? 9999 : bIndex;

    if (safeA !== safeB) {
        return safeA - safeB;
    }

    // Sorting by machine_no
    if (a.machine_no !== b.machine_no) {
        return a.machine_no - b.machine_no;
    }

    // Sorting by nsds (Day before Night)
    const shiftOrder = shift => shift.toLowerCase() === 'dayshift' ? 0 : 1;
    return shiftOrder(a.nsds) - shiftOrder(b.nsds);
});
    ...
    tr.innerHTML += `<td rowspan="${rowspan}" style="position: sticky; left: 738px; z-index: 10; background: #fff; text-align:center; vertical-align:middle;">${row.skill_level}</td>`;
